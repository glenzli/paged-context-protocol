    ...
    pub async fn pack_pages(
        ...
        self.run("Page packing", move |mut connection| {
            ...
            for input in &inputs {
                ...
                if !input.is_anchor() {
                    ensure_unreferenced_leaf(
                        &transaction,
                        &input.exact.page_id,
                        &input.exact.revision_id,
                    )?;
                }
            }
        ...
        .query_row(
            ...
            SELECT EXISTS (
                ...
                WHERE from_page_id = ?1 OR to_page_id = ?1
                   OR EXISTS (
                       SELECT 1 FROM json_each(basis_revision_ids_json) WHERE value = ?2
                   )
                UNION ALL
                SELECT 1 FROM pcp_relation_retractions
                WHERE from_revision_id = ?2 OR to_revision_id = ?2
                UNION ALL
                SELECT 1 FROM pcp_provenance_inputs
                WHERE derived_revision_id = ?2 OR input_revision_id = ?2
                UNION ALL
                SELECT 1 FROM pcp_summaries
                WHERE target_revision_id = ?2 OR summary_page_id = ?1
                UNION ALL
                SELECT 1 FROM pcp_summary_assessments WHERE target_revision_id = ?2
                UNION ALL
                SELECT 1 FROM pcp_validity_assessments
                WHERE target_revision_id = ?2
                   OR EXISTS (
                       SELECT 1 FROM json_each(basis_revision_ids_json) WHERE value = ?2
                   )
                UNION ALL
                SELECT 1 FROM pcp_revision_retention_leases WHERE revision_id = ?2
            )
            ",
            params![page_id, revision_id],
            |row| row.get(0),
        )
        .context("check PCP packing references")?;
    anyhow::ensure!(
        !referenced,
        "PCP packing input is already referenced or explicitly retained"
    );
    Ok(())
}
