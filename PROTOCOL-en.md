...
Validity assessments use revisioned Pages for audit, but are not independent knowledge: exclude them from ordinary page lists, default search, semantic indexing and maintenance candidates derived from those inventories. Exact reads and Validity/History projections remain available under normal permissions. Page lifecycle controls discovery; validity standings and evidence describe applicability.
...
`submit_feedback` records an explicit user or tenant challenge and does not directly mutate a recalled Page. `challengedRevisionIds` identifies exact disputed Revisions; `usedRevisionIds` records the complete exact PCP evidence used by the challenged response and may be a superset. Optional `evidenceRevisionIds` records additional correction evidence, including a new Page written after that response; it MUST NOT be mislabeled as actually-used context. Feedback requires `ingest` in its destination Scope and `read_detail` for every referenced Revision. Cross-Scope feedback references require neither target writes, `assess`, nor `derive_across_scopes`, and grant none of them. Publishing derived content across Scopes still requires the existing derivation authorization. `responseRef` and SourceRefs remain opaque tenant-owned locators: PCP does not dereference, parse, search, or render the external source. Each challenged Revision in a multi-target signal is reconciled independently. Context-only or evidence-only Revisions MUST NOT be promoted into reconciliation targets. Claimed `authority` is review context rather than a fact authenticated by PCP.
...
Reconciliation is atomic at one exact target Revision and its bounded evidence. Optional `feedbackRevisionId` binds a feedback-driven decision to a challenged target; discovery from ordinary writes may omit it and MUST NOT fabricate user feedback. The disposition is `no_source_change`, `qualified`, `disputed`, `superseded`, or `retracted`. Only `superseded` may name a replacement Revision present in the evidence; Store writes Validity and the cross-Page `supersedes` Relation in one transaction. It MUST revalidate current target and replacement Revisions and the reviewed Validity head. An absent `expectedAssessmentRevisionId` means there was no assessment at review time, not permission to overwrite a newer one.

`rationale` is optional. Clear replacement or withdrawal decisions are expressed by standing, exact evidence, Relations, actor and timestamp without repeating the content. Use a concise rationale and scope when qualifications or disagreements need explanation. Model proposals still need reviewable grounds; review explanations do not become independent knowledge.

When a local administrator repairs feedback content, the new Revision and its feedback index binding must be published in one transaction, preserving challenged, used and corrective-evidence references. Only the current active feedback Revision is actionable. Pending proposals and staged approvals based on older feedback become stale; unresolved targets require analysis of the revised content. Preserve old feedback and applied per-target results for audit. Editing feedback does not undo or replay applied Validity or Relations; changing those decisions requires separate review.

Ordinary writes do not assert replacement. Similarity, shared provenance and chronology can identify candidates but cannot establish correctness; a partial correction must not discard unrelated useful claims. A deployment may automatically apply low-impact, same-Scope feedback decisions. Cross-Scope reconciliation, updates discovered from ordinary writes, `superseded`, and `retracted` require approval by an authorized operator. Pending proposals change neither default recall nor original content. Approval does not expand client read permissions or imply that all dependent summaries have been rewritten; affected derived Revisions are inputs to subsequent maintenance.
